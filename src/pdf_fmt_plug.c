...
char (*pdf_key)[PLAINTEXT_LENGTH + 1];

static void init(struct fmt_main *self)
{
	omp_autotune(self, OMP_SCALE);

	pdf_key = mem_calloc(sizeof(*pdf_key), self->params.max_keys_per_crypt);
	any_pdf_cracked = 0;
...
{
	strnzcpy(pdf_key[index], key, sizeof(*pdf_key));
}

static char *get_key(int index)
{
	return pdf_key[index];
}
...
#pragma omp parallel for
	...
	for (index = 0; index < count; index++) {
		...
		pdf_compute_user_password((unsigned char*)pdf_key[index], output);
